<?php
/*
Plugin Name: BLMD Simple Local Avatars
Plugin URI: https://github.com/blmd/blmd-simple-local-avatars
Description: Local avatars, fork of http://10up.com/plugins/simple-local-avatars-wordpress/
Author: blmd
Author URI: https://github.com/blmd
License: GPLv2 or later
Version: 0.2

GitHub Plugin URI: https://github.com/blmd/blmd-simple-local-avatars
// Original author: Jake Goldman, 10up
*/

class BLMD_Simple_Local_Avatars {
	...
	public function get_avatar( $avatar = '', $id_or_email, $size = 96, $default = '', $alt = '' ) {
		...
		if ( ! array_key_exists( $size, $local_avatars ) ) {
			...
			$local_avatars[$retina_size] = $local_avatars['full']; // just in case of failure elsewhere
			...
			if ( $allow_dynamic_resizing = apply_filters( 'simple_local_avatars_dynamic_resize', true ) ) :
				...
				if ( ! is_wp_error( $editor ) ) {
					...
					$editor = wp_get_image_editor( $avatar_full_path );
					$resized = $editor->resize( $retina_size, $retina_size, true );
					if ( ! is_wp_error( $resized ) ) {
						$dest_file = $editor->generate_filename();
						$saved = $editor->save( $dest_file );
						if ( ! is_wp_error( $saved ) )
							$local_avatars[$retina_size] = str_replace( $upload_path['basedir'], $upload_path['baseurl'], $dest_file );
					}
				}
		...
		if ( !isset( $local_avatars[$retina_size] ) ) { $local_avatars[$retina_size] = null; }
		if ( 'http' != substr( $local_avatars[$retina_size], 0, 4 ) ) {
			$local_avatars[$retina_size] = home_url( $local_avatars[$retina_size] );
		}


		$author_class = is_author( $user_id ) ? ' current-author' : '' ;
		$avatar = "<img alt='" . esc_attr( $alt ) . "' src='" . esc_url( $local_avatars[$size] ) . "' srcset='" . esc_url( $local_avatars[$retina_size] ) . " 2x' class='avatar avatar-{$size}{$author_class} photo' height='{$size}' width='{$size}' />";
			...
			<td style="width: 50px;" id="simple-local-avatar-photo">
				<?php
					...
					if ( $a_sizes = get_theme_support( 'simple_local_avatars_sizes' ) ) {
						if ( !empty( $a_sizes[0] ) && is_array( $a_sizes[0] ) ) { $a_sizes = $a_sizes[0]; }
						foreach ( (array)$a_sizes as $a_size ) {
							echo '<div style="display:table-cell">';
							echo get_simple_local_avatar( $profileuser->ID, $a_size );
							get_simple_local_avatar( $profileuser->ID, intval($a_size)*2 );
							echo '</div>';
						}
					}
					else {
...
$simple_local_avatars = new BLMD_Simple_Local_Avatars;
...
if ( !function_exists( 'simple_local_avatars_uninstall' ) ):
